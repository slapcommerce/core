...
describe('CreateScheduleService', () => {
  test('should successfully create schedule with command data', async () => {
    ...
    try {
      ...
      expect(events[0].eventType).toBe('schedule.created')
      ...
      expect(outboxEvents[0].eventType).toBe('schedule.created')
    } finally {
      batcher.stop()
  ...
  test('should save event with correct data structure', async () => {
    ...
    try {
      ...
      expect(event.aggregateId).toBe(scheduleId)
      expect(event.correlationId).toBe(correlationId)
      expect(event.version).toBe(0)
      expect(event.userId).toBe('user-event-test')
  ...
  test('should create multiple independent schedules', async () => {
    ...
    try {
      ...
      const ids = snapshots.map(s => s.aggregateId)
