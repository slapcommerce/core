...
import { ScheduleDropshipVariantVisibleDropService } from '../../../../../../src/api/app/dropshipVariant/commands/admin/scheduleDropshipVariantVisibleDropService'
import { setupTestEnvironment, createTestProduct, createTestVariant } from './helpers'

describe('ScheduleDropshipVariantVisibleDropService', () => {
  test('should set variant to visible pending drop status', async () => {
    ...
    try {
      ...
      const service = new ScheduleDropshipVariantVisibleDropService(unitOfWork)

      const scheduledFor = new Date(Date.now() + 86400000)
      const result = await service.execute({
        type: 'scheduleDropshipVariantVisibleDrop',
        id: variant.id,
  ...
  test('should create visible drop scheduled event', async () => {
    ...
    try {
      ...
      expect(eventTypes).toContain('dropship_variant.visible_drop_scheduled')
    } finally {
      batcher.stop()
      closeTestDatabase(db)
    }
