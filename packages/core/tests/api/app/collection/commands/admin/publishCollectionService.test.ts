...
import type { PublishCollectionCommand } from '../../../../../../src/api/app/collection/commands/admin/commands'
...
describe('PublishCollectionService', () => {
  test('should successfully publish draft collection', async () => {
    // Arrange
    ...
    try {
      ...
      const service = new PublishCollectionService(unitOfWork)
      const command: PublishCollectionCommand = {
      ...
      const events = db.query(`
        SELECT * FROM events
        WHERE aggregate_id = ? AND eventType = 'collection.published'
  ...
  test('should throw error when collection is already published', async () => {
    // Arrange
    const { db, batcher, unitOfWork } = await setupTestEnvironment()
    ...
    try {
      ...
      const command: PublishCollectionCommand = {
        ...
        id: 'collection-123',
        userId: 'user-456',
        expectedVersion: 1, // Version 1 after initial publish
      }
  ...
  test('should throw error when collection is archived', async () => {
    ...
    try {
      ...
      const command: PublishCollectionCommand = {
        ...
        id: 'collection-123',
        userId: 'user-456',
        expectedVersion: 1, // Version 1 after archive
      }
  ...
  test('should preserve other collection fields when publishing', async () => {
    // Arrange
    const { db, batcher, unitOfWork } = await setupTestEnvironment()
  ...
  test('should add outbox event when publishing', async () => {
    // Arrange
    const { db, batcher, unitOfWork } = await setupTestEnvironment()
