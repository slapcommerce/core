...
import { UpdateCollectionMetadataService } from '../../../../src/api/app/collection/updateCollectionMetadataService'
...
import type { UpdateCollectionMetadataCommand } from '../../../../src/api/app/collection/commands'
...
async function createCollectionInDatabase(
  ...
  await unitOfWork.withTransaction(async ({ snapshotRepository, eventRepository, outboxRepository }) => {
    ...
    snapshotRepository.saveSnapshot({
      ...
      correlation_id: slugAggregate.id,
  ...
  test('should successfully update metadata without slug change', async () => {
    ...
    try {
      ...
      const events = db.query(`
        SELECT * FROM events
        WHERE aggregate_id = ? AND event_type = 'collection.metadata_updated'
