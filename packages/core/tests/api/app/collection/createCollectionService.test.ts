...
import type { CreateCollectionCommand } from '../../../../src/api/app/collection/commands'
...
describe('CreateCollectionService', () => {
  test('should successfully create collection with available slug', async () => {
    ...
    try {
      ...
      expect(collectionEvents[0].event_type).toBe('collection.created')
  ...
  test('should save event with correct data structure', async () => {
    ...
    try {
      ...
      const event = db.query(`
        SELECT * FROM events
        WHERE aggregate_id = ? AND event_type = 'collection.created'
      `).get(command.id) as any
  ...
  test('should create unique collection IDs for different collections', async () => {
    ...
    try {
      ...
      const collectionIds = snapshots.map(s => s.aggregate_id)
