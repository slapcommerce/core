...
import { ScheduleDigitalDownloadableVariantHiddenDropService } from '../../../../../../src/api/app/digitalDownloadableVariant/commands/admin/scheduleDigitalDownloadableVariantHiddenDropService'
import { setupTestEnvironment, createTestProduct, createTestVariant } from './helpers'

describe('ScheduleDigitalDownloadableVariantHiddenDropService', () => {
  test('should set variant to hidden pending drop status', async () => {
    ...
    try {
      ...
      const service = new ScheduleDigitalDownloadableVariantHiddenDropService(unitOfWork)

      const scheduledFor = new Date(Date.now() + 86400000)
      const result = await service.execute({
        type: 'scheduleDigitalDownloadableVariantHiddenDrop',
        id: variant.id,
  ...
  test('should create hidden drop scheduled event', async () => {
    ...
    try {
      ...
      expect(eventTypes).toContain('digital_downloadable_variant.hidden_drop_scheduled')
    } finally {
      batcher.stop()
      closeTestDatabase(db)
    }
