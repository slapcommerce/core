...
import { ScheduleDigitalDownloadableProductVisibleDropService } from '../../../../../../src/api/app/digitalDownloadableProduct/commands/admin/scheduleDigitalDownloadableProductVisibleDropService'
import { setupTestEnvironment, createTestProduct } from './helpers'

describe('ScheduleDigitalDownloadableProductVisibleDropService', () => {
  test('should set product to visible pending drop status', async () => {
    const { db, batcher, unitOfWork } = await setupTestEnvironment()

    try {
      const product = await createTestProduct(unitOfWork)
      const service = new ScheduleDigitalDownloadableProductVisibleDropService(unitOfWork)

      const scheduledFor = new Date(Date.now() + 86400000) // 1 day from now
      const result = await service.execute({
        type: 'scheduleDigitalDownloadableProductVisibleDrop',
        id: product.id,
  ...
  test('should create visible drop scheduled event', async () => {
    ...
    try {
      ...
      expect(eventTypes).toContain('digital_downloadable_product.visible_drop_scheduled')
    } finally {
      batcher.stop()
      closeTestDatabase(db)
    }
