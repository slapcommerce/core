...
import { RemoveVariantImageService } from '../../../../../../src/api/app/variant/commands/admin/removeVariantImageService'
import { VariantAggregate } from '../../../../../../src/api/domain/variant/aggregate'
import type { RemoveVariantImageCommand } from '../../../../../../src/api/app/variant/commands/admin/commands'
...
function createValidCommand(): RemoveVariantImageCommand {
  return {
  ...
  await unitOfWork.withTransaction(async ({ snapshotRepository, eventRepository, outboxRepository }) => {
    ...
    const images = variantAggregate.images
      .addImage({
        imageId: 'image-1',
        urls: {
          thumbnail: { original: 'url1', webp: 'url1', avif: 'url1' },
          small: { original: 'url1', webp: 'url1', avif: 'url1' },
          medium: { original: 'url1', webp: 'url1', avif: 'url1' },
          large: { original: 'url1', webp: 'url1', avif: 'url1' },
          original: { original: 'url1', webp: 'url1', avif: 'url1' },
        },
      }, 'First image')
      .addImage({
        imageId: 'image-2',
        urls: {
          thumbnail: { original: 'url2', webp: 'url2', avif: 'url2' },
          small: { original: 'url2', webp: 'url2', avif: 'url2' },
          medium: { original: 'url2', webp: 'url2', avif: 'url2' },
          large: { original: 'url2', webp: 'url2', avif: 'url2' },
          original: { original: 'url2', webp: 'url2', avif: 'url2' },
        },
      }, 'Second image')
...
describe('RemoveVariantImageService', () => {
  test('should successfully remove image from variant', async () => {
    ...
    try {
      ...
      command.expectedVersion = 1 // After adding images, version is 1
      await createVariantWithImagesInDatabase(unitOfWork, command.id)

      const service = new RemoveVariantImageService(unitOfWork)
      ...
      expect(variantPayload.images[0].imageId).toBe('image-2')
  ...
  test('should increment version after removing image', async () => {
    // Arrange
    const { db, batcher, unitOfWork } = await setupTestEnvironment()
  ...
  test('should remove all images when removing last image', async () => {
    // Arrange
    const { db, batcher, unitOfWork } = await setupTestEnvironment()
    ...
    try {
      ...
      const variantPayload = JSON.parse(variantSnapshot.payload)
      expect(variantPayload.images).toHaveLength(0)
    } finally {
      batcher.stop()
      closeTestDatabase(db)
    }
