...
import { UpdateProductMetadataService } from '../../../../../../src/api/app/product/commands/admin/updateProductMetadataService'
import { ProductAggregate } from '../../../../../../src/api/domain/product/aggregate'
import type { UpdateProductMetadataCommand } from '../../../../../../src/api/app/product/commands/admin/commands'
...
describe('UpdateProductMetadataService', () => {
  test('should successfully update product metadata', async () => {
    ...
    try {
      ...
      const service = new UpdateProductMetadataService(unitOfWork)
      const command: UpdateProductMetadataCommand = {
      ...
      expect(events[0].eventType).toBe('product.metadata_updated')
  ...
  test('should save event with correct priorState and newState', async () => {
    ...
    try {
      ...
      expect(eventPayload.priorState.metaTitle).toBe(productParams.metaTitle)
      expect(eventPayload.priorState.metaDescription).toBe(productParams.metaDescription)
  ...
  test('should preserve other product fields when updating metadata', async () => {
    // Arrange
    const { db, batcher, unitOfWork } = await setupTestEnvironment()
