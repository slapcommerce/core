...
import { UpdateProductTaxDetailsService } from '../../../../../../src/api/app/product/commands/admin/updateProductTaxDetailsService'
import { ProductAggregate } from '../../../../../../src/api/domain/product/aggregate'
import type { UpdateProductTaxDetailsCommand } from '../../../../../../src/api/app/product/commands/admin/commands'
...
describe('UpdateProductTaxDetailsService', () => {
  test('should successfully update product tax details', async () => {
    // Arrange
    ...
    try {
      ...
      const service = new UpdateProductTaxDetailsService(unitOfWork)
      const command: UpdateProductTaxDetailsCommand = {
      ...
      expect(events[0].eventType).toBe('product.update_product_tax_details')
  ...
  test('should save event with correct priorState and newState', async () => {
    ...
    try {
      ...
      expect(eventPayload.priorState.taxable).toBe(true)
      expect(eventPayload.priorState.taxId).toBe('OLD-TAX-ID')
      expect(eventPayload.newState.taxable).toBe(false)
      expect(eventPayload.newState.taxId).toBe('NEW-TAX-ID')
    } finally {
      batcher.stop()
      closeTestDatabase(db)
    }
  ...
  test('should preserve other product fields when updating tax details', async () => {
    // Arrange
    const { db, batcher, unitOfWork } = await setupTestEnvironment()
