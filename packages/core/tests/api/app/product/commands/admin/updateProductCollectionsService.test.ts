...
import { UpdateProductCollectionsService } from '../../../../../../src/api/app/product/commands/admin/updateProductCollectionsService'
import { ProductAggregate } from '../../../../../../src/api/domain/product/aggregate'
import type { UpdateProductCollectionsCommand } from '../../../../../../src/api/app/product/commands/admin/commands'
...
describe('UpdateProductCollectionsService', () => {
  test('should successfully update product collections', async () => {
    ...
    try {
      ...
      const service = new UpdateProductCollectionsService(unitOfWork)
      const command: UpdateProductCollectionsCommand = {
        type: 'updateProductCollections',
        id: productParams.id,
        collectionIds: ['collection-3', 'collection-4', 'collection-5'],
        userId: 'user-456',
        expectedVersion: 0,
      }
      ...
      expect(payload.collectionIds).toEqual(['collection-3', 'collection-4', 'collection-5'])
      ...
      expect(events[0].eventType).toBe('product.collections_updated')
  ...
  test('should save event with correct priorState and newState', async () => {
    ...
    try {
      ...
      const command: UpdateProductCollectionsCommand = {
        type: 'updateProductCollections',
        id: productParams.id,
        collectionIds: ['collection-3', 'collection-4'],
        userId: 'user-456',
        expectedVersion: 0,
      }
      ...
      expect(eventPayload.priorState.collectionIds).toEqual(productParams.collectionIds)
      expect(eventPayload.newState.collectionIds).toEqual(['collection-3', 'collection-4'])
    } finally {
      batcher.stop()
      closeTestDatabase(db)
    }
  ...
  test('should handle multiple sequential updates', async () => {
    ...
    try {
      ...
      expect(payload.collectionIds).toEqual(['collection-b', 'collection-c'])
