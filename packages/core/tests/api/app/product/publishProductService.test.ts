...
import { PublishProductService } from '../../../../src/api/app/product/publishProductService'
import { ProductAggregate } from '../../../../src/api/domain/product/aggregate'
import type { PublishProductCommand } from '../../../../src/api/app/product/commands/commands'
...
describe('PublishProductService', () => {
  test('should successfully publish a product', async () => {
    ...
    try {
      ...
      expect(events[0].event_type).toBe('product.published')
  ...
  test('should set status to active when published', async () => {
    // Arrange
    const { db, batcher, unitOfWork } = await setupTestEnvironment()
  ...
  test('should set publishedAt timestamp when published', async () => {
    // Arrange
    const { db, batcher, unitOfWork } = await setupTestEnvironment()
    ...
    try {
      ...
      const publishedAt = new Date(payload.publishedAt).getTime()
  ...
  test('should preserve other product fields when publishing', async () => {
    // Arrange
    const { db, batcher, unitOfWork } = await setupTestEnvironment()
