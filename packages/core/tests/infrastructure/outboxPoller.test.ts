...
import {
  OutboxPoller,
  type OutboxHandler,
} from "../../src/infrastructure/outboxPoller";
...
describe("OutboxPoller", () => {
  ...
  test("should retry failed handlers with exponential backoff", async () => {
    ...
    try {
      ...
      expect(new Date(processing.next_retry_at).getTime()).toBeGreaterThan(
        Date.now(),
      );
  ...
  test("should handle independent retries when one handler succeeds and another fails", async () => {
    ...
    try {
      ...
      expect(new Date(smsProcessing.next_retry_at).getTime()).toBeGreaterThan(
        Date.now(),
      );
