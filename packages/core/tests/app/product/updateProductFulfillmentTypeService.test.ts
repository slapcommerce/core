...
import { UpdateProductFulfillmentTypeService } from '../../../src/app/product/updateProductFulfillmentTypeService'
import { ProductAggregate } from '../../../src/domain/product/aggregate'
import { VariantAggregate } from '../../../src/domain/variant/aggregate'
import type { UpdateProductFulfillmentTypeCommand } from '../../../src/app/product/commands'
...
async function createVariantInDatabase(unitOfWork: UnitOfWork, variantId: string, productId: string) {
  await unitOfWork.withTransaction(async ({ snapshotRepository, eventRepository, outboxRepository }) => {
    ...
    snapshotRepository.saveSnapshot({
      ...
      correlation_id: `correlation-${variantId}`,
...
describe('UpdateProductFulfillmentTypeService', () => {
  test('should successfully update product fulfillment type', async () => {
    ...
    try {
      ...
      expect(events[0].event_type).toBe('product.fulfillment_type_updated')
