...
import type { SetDigitalDownloadableProductDefaultVariantCommand } from "./commands";
...
export class SetDigitalDownloadableProductDefaultVariantService {
  constructor(private unitOfWork: UnitOfWork) {
    this.unitOfWork = unitOfWork;
  }

  async execute(command: SetDigitalDownloadableProductDefaultVariantCommand) {
    return await this.unitOfWork.withTransaction(async (repositories) => {
      ...
      if (!productSnapshot) {
        throw new Error(
          `Digital downloadable product with id ${command.productId} not found`
        );
      }
      ...
      const productAggregate =
        DigitalDownloadableProductAggregate.loadFromSnapshot(productSnapshot);
