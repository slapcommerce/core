...
import type { CreateDigitalDownloadableProductCommand } from "./commands";
...
export class CreateDigitalDownloadableProductService
  implements Service<CreateDigitalDownloadableProductCommand>
{
  constructor(private unitOfWork: UnitOfWork) {
    this.unitOfWork = unitOfWork;
  }

  async execute(command: CreateDigitalDownloadableProductCommand) {
    return await this.unitOfWork.withTransaction(async (repositories) => {
      ...
      const productAggregate = DigitalDownloadableProductAggregate.create({
        id: command.id,
        correlationId: command.correlationId,
        ...
        taxId: command.taxId,
        maxDownloads: command.maxDownloads ?? null,
        accessDurationDays: command.accessDurationDays ?? null,
      });
