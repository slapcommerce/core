...
import type { UnpublishDigitalDownloadableProductCommand } from "./commands";
import { DigitalDownloadableProductAggregate } from "../../../../domain/digitalDownloadableProduct/aggregate";
import { randomUUIDv7 } from "bun";
import type { Service } from "../../../service";

export class UnpublishDigitalDownloadableProductService
  implements Service<UnpublishDigitalDownloadableProductCommand>
{
  constructor(private unitOfWork: UnitOfWork) {
    this.unitOfWork = unitOfWork;
  }

  async execute(command: UnpublishDigitalDownloadableProductCommand) {
    return await this.unitOfWork.withTransaction(async (repositories) => {
      ...
      productAggregate.unpublish(command.userId);
