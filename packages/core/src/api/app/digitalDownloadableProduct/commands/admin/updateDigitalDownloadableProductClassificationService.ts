...
import type { UpdateDigitalDownloadableProductClassificationCommand } from "./commands";
import { DigitalDownloadableProductAggregate } from "../../../../domain/digitalDownloadableProduct/aggregate";
import { randomUUIDv7 } from "bun";
import type { Service } from "../../../service";

export class UpdateDigitalDownloadableProductClassificationService
  implements Service<UpdateDigitalDownloadableProductClassificationCommand>
{
  constructor(private unitOfWork: UnitOfWork) {
    this.unitOfWork = unitOfWork;
  }

  async execute(command: UpdateDigitalDownloadableProductClassificationCommand) {
    return await this.unitOfWork.withTransaction(async (repositories) => {
      ...
      productAggregate.updateVendor(command.vendor, command.userId);
