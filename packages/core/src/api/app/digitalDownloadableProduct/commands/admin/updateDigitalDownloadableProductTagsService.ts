...
import type { UpdateDigitalDownloadableProductTagsCommand } from "./commands";
import { DigitalDownloadableProductAggregate } from "../../../../domain/digitalDownloadableProduct/aggregate";
import { randomUUIDv7 } from "bun";
import type { Service } from "../../../service";

export class UpdateDigitalDownloadableProductTagsService
  implements Service<UpdateDigitalDownloadableProductTagsCommand>
{
  constructor(private unitOfWork: UnitOfWork) {
    this.unitOfWork = unitOfWork;
  }

  async execute(command: UpdateDigitalDownloadableProductTagsCommand) {
    return await this.unitOfWork.withTransaction(async (repositories) => {
      ...
      productAggregate.updateTags(command.tags, command.userId);
