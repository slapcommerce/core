...
import type { UpdateDigitalDownloadableProductTaxDetailsCommand } from "./commands";
import { DigitalDownloadableProductAggregate } from "../../../../domain/digitalDownloadableProduct/aggregate";
import { randomUUIDv7 } from "bun";
import type { Service } from "../../../service";

export class UpdateDigitalDownloadableProductTaxDetailsService
  implements Service<UpdateDigitalDownloadableProductTaxDetailsCommand>
{
  constructor(private unitOfWork: UnitOfWork) {
    this.unitOfWork = unitOfWork;
  }

  async execute(command: UpdateDigitalDownloadableProductTaxDetailsCommand) {
    return await this.unitOfWork.withTransaction(async (repositories) => {
      ...
      productAggregate.updateTaxDetails(
        command.taxable,
        command.taxId,
        command.userId
      );
