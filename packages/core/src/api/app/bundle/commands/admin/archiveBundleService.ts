...
import type { ArchiveBundleCommand } from "./commands";
import { BundleAggregate } from "../../../../domain/bundle/aggregate";
import { randomUUIDv7 } from "bun";
import type { Service } from "../../../service";

export class ArchiveBundleService implements Service<ArchiveBundleCommand> {
  constructor(private unitOfWork: UnitOfWork) {
    this.unitOfWork = unitOfWork;
  }

  async execute(command: ArchiveBundleCommand) {
    return await this.unitOfWork.withTransaction(async (repositories) => {
      ...
      bundleAggregate.archive(command.userId);
