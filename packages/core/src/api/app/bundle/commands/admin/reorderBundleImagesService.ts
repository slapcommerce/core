...
import type { ReorderBundleImagesCommand } from "./commands";
import { BundleAggregate } from "../../../../domain/bundle/aggregate";
import { randomUUIDv7 } from "bun";
import type { Service } from "../../../service";

export class ReorderBundleImagesService implements Service<ReorderBundleImagesCommand> {
  constructor(private unitOfWork: UnitOfWork) {
    this.unitOfWork = unitOfWork;
  }

  async execute(command: ReorderBundleImagesCommand) {
    return await this.unitOfWork.withTransaction(async (repositories) => {
      ...
      bundleAggregate.reorderImages(command.orderedImageIds, command.userId);
