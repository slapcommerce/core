...
import type { UpdateBundleImageAltTextCommand } from "./commands";
import { BundleAggregate } from "../../../../domain/bundle/aggregate";
import { randomUUIDv7 } from "bun";
import type { Service } from "../../../service";

export class UpdateBundleImageAltTextService implements Service<UpdateBundleImageAltTextCommand> {
  constructor(private unitOfWork: UnitOfWork) {
    this.unitOfWork = unitOfWork;
  }

  async execute(command: UpdateBundleImageAltTextCommand) {
    return await this.unitOfWork.withTransaction(async (repositories) => {
      ...
      bundleAggregate.updateImageAltText(command.imageId, command.altText, command.userId);
