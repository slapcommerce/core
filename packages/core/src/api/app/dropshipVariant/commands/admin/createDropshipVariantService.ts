...
import type { CreateDropshipVariantCommand } from "./commands";
...
export class CreateDropshipVariantService implements Service<CreateDropshipVariantCommand> {

  constructor(private unitOfWork: UnitOfWork) {}

  async execute(command: CreateDropshipVariantCommand) {
    return await this.unitOfWork.withTransaction(async (repositories) => {
      ...
      if (!productSnapshot) {
        throw new Error(`Dropship product with id ${command.productId} not found`);
      }
      ...
      const variantAggregate = DropshipVariantAggregate.create({
        id: command.id,
        correlationId: command.correlationId,
        ...
        supplierCost: command.supplierCost,
