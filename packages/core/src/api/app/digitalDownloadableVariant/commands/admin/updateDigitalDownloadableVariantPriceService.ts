...
import type { UpdateDigitalDownloadableVariantPriceCommand } from "./commands";
import { DigitalDownloadableVariantAggregate } from "../../../../domain/digitalDownloadableVariant/aggregate";
import { randomUUIDv7 } from "bun";
import type { Service } from "../../../service";

export class UpdateDigitalDownloadableVariantPriceService implements Service<UpdateDigitalDownloadableVariantPriceCommand> {

  constructor(private unitOfWork: UnitOfWork) {}

  async execute(command: UpdateDigitalDownloadableVariantPriceCommand) {
    return await this.unitOfWork.withTransaction(async (repositories) => {
      ...
      variantAggregate.updatePrice(command.price, command.userId);
