...
import type { ArchiveDigitalDownloadableVariantCommand } from "./commands";
...
export class ArchiveDigitalDownloadableVariantService implements Service<ArchiveDigitalDownloadableVariantCommand> {

  constructor(private unitOfWork: UnitOfWork) {}

  async execute(command: ArchiveDigitalDownloadableVariantCommand) {
    return await this.unitOfWork.withTransaction(async (repositories) => {
      ...
      if (!productSnapshot) {
        throw new Error(`Digital downloadable product ${variantSnapshotData.productId} not found`);
      }
