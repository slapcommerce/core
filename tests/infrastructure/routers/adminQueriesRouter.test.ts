...
import { createAdminQueriesRouter } from '../../../src/infrastructure/routers/adminQueriesRouter'
import { schemas } from '../../../src/infrastructure/schemas'

describe('createAdminQueriesRouter', () => {
  let db: Database
  let router: ReturnType<typeof createAdminQueriesRouter>
  ...
  beforeEach(() => {
    ...
    router = createAdminQueriesRouter(db)
  })
  ...
  test('should execute productVariantsView query with productId filter', async () => {
    // Arrange
    const productId1 = randomUUIDv7()
    const productId2 = randomUUIDv7()
    ...
    // Act
    const result = await router('productVariantsView', { productId: productId1 })
  ...
  test('should execute slugRedirectsView query with oldSlug filter', async () => {
    // Arrange
    const productId = randomUUIDv7()
    const now = new Date().toISOString()
    ...
    db.run(
      `INSERT INTO slug_redirects (old_slug, new_slug, product_id, created_at)
       VALUES (?, ?, ?, ?)`,
      ['old-slug-1', 'new-slug-1', productId, now]
    )
    db.run(
      `INSERT INTO slug_redirects (old_slug, new_slug, product_id, created_at)
       VALUES (?, ?, ?, ?)`,
      ['old-slug-2', 'new-slug-2', productId, now]
    )

    // Act
    const result = await router('slugRedirectsView', { oldSlug: 'old-slug-1' })
  ...
  test('should handle database errors gracefully', async () => {
    ...
    const brokenRouter = createAdminQueriesRouter(newDb)
    newDb.close()
