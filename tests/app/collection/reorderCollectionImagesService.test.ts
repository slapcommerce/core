...
import { ReorderCollectionImagesService } from '../../../src/app/collection/reorderCollectionImagesService'
import { UnitOfWork } from '../../../src/infrastructure/unitOfWork'
import { TransactionBatcher } from '../../../src/infrastructure/transactionBatcher'
import { schemas } from '../../../src/infrastructure/schemas'
import { ProjectionService } from '../../../src/infrastructure/projectionService'
import type { CreateCollectionCommand, AddCollectionImageCommand, ReorderCollectionImagesCommand } from '../../../src/app/collection/commands'
...
function createReorderCommand(overrides?: Partial<ReorderCollectionImagesCommand>): ReorderCollectionImagesCommand {
  return {
    id: overrides?.id ?? randomUUIDv7(),
    userId: overrides?.userId ?? randomUUIDv7(),
...
describe('ReorderCollectionImagesService', () => {
  test('should successfully reorder images in collection', async () => {
    // Arrange
    ...
    const reorderService = new ReorderCollectionImagesService(unitOfWork, projectionService)
  ...
  test('should throw error when collection does not exist', async () => {
    ...
    // Act & Assert
    await expect(reorderService.execute(reorderCommand)).rejects.toThrow('Collection with id')
  ...
  test('should throw error when ordered image IDs do not match collection images', async () => {
    // Arrange
    const db = new Database(':memory:')
    for (const schema of schemas) {
      db.run(schema)
