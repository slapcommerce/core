...
import { UpdateVariantSkuService } from '../../../src/app/variant/updateVariantSkuService'
...
import type { UpdateVariantSkuCommand } from '../../../src/app/variant/commands'
...
describe('UpdateVariantSkuService', () => {
  test('should successfully update variant SKU', async () => {
    ...
    const updateService = new UpdateVariantSkuService(unitOfWork, projectionService)

    // Act
    await updateService.execute(updateCommand)

    // Assert - Verify variant.sku_updated event was saved
    await new Promise(resolve => setTimeout(resolve, 100))
    const events = db.query('SELECT * FROM events WHERE aggregate_id = ? ORDER BY version ASC').all(variantCommand.id) as any[]
    expect(events.length).toBe(2)
    expect(events[1]!.event_type).toBe('variant.sku_updated')
    expect(events[1]!.version).toBe(1)

    const eventPayload = JSON.parse(events[1]!.payload)
    expect(eventPayload.newState.sku).toBe('SKU-456')
    expect(eventPayload.priorState.sku).toBe('SKU-123')
    ...
    expect(snapshotPayload.sku).toBe('SKU-456')
  ...
  test('should throw error when variant not found', async () => {
    ...
    const unitOfWork = new UnitOfWork(db, batcher)
    const projectionService = new ProjectionService()
    const service = new UpdateVariantSkuService(unitOfWork, projectionService)
    const command: UpdateVariantSkuCommand = {
      id: randomUUIDv7(),
      userId: randomUUIDv7(),
      sku: 'SKU-456',
      expectedVersion: 0,
  ...
  test('should update SKU to different format', async () => {
    ...
    // Assert - Verify SKU was updated to new format
    await new Promise(resolve => setTimeout(resolve, 100))
    const snapshot = db.query('SELECT * FROM snapshots WHERE aggregate_id = ?').get(variantCommand.id) as any
    const snapshotPayload = JSON.parse(snapshot.payload)
    expect(snapshotPayload.sku).toBe('NEW-FORMAT-789')
