...
import type { UpdateProductShippingSettingsCommand } from "./commands";
import type { ProjectionService } from "../../infrastructure/projectionService";
import { ProductAggregate } from "../../domain/product/aggregate";
import { randomUUIDv7 } from "bun";

export class UpdateProductShippingSettingsService {
  constructor(
  ...
  async execute(command: UpdateProductShippingSettingsCommand) {
    return await this.unitOfWork.withTransaction(async (repositories) => {
      ...
      productAggregate.updateShippingSettings(command.requiresShipping, command.taxable);
