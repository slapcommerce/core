...
async function getProductMetadata(
  ...
  const { snapshotRepository, productListViewRepository } = repositories
...
export const productVariantProjection: ProjectionHandler = async (
  event: DomainEvent<string, Record<string, unknown>>,
  repositories: UnitOfWorkRepositories
): Promise<void> => {
  const { snapshotRepository, productVariantRepository, productListViewRepository } = repositories
  switch (event.eventName) {
    case "variant.created": {
      ...
      const productData = await getProductMetadata(variantState.productId, repositories)
      if (!productData) {
        break
      }
    ...
    case "product.page_layout_updated": {
      ...
      const productEvent = event as ProductCreatedEvent | ProductArchivedEvent | ProductPublishedEvent | ProductDetailsUpdatedEvent | ProductMetadataUpdatedEvent | ProductClassificationUpdatedEvent | ProductTagsUpdatedEvent | ProductShippingSettingsUpdatedEvent | ProductPageLayoutUpdatedEvent
      ...
      const productData = await getProductMetadata(productId, repositories)
      if (!productData) {
        break
      }
